...
@TestProfile(RepositoryCommandResourceUpdateStatusInvalidStatusIT.BuildTimeValueChangeTestProfile.class)
class RepositoryCommandResourceUpdateStatusInvalidStatusIT {
	...
	@Test
	@DisplayName("When a PUT request is made with a key as parameter and with a new invalid status in the body, I expect a response code 400 with an empty "
			+ "body")
	void expect_response_with_status_400_and_empty_body_when_calling_put_with_key_as_parameter_and_new_invalid_status_in_the_body() throws IOException {
		// GIVEN
		final var testParams = mockForPutWithKeyAsParameterAndInvalidStatus();
		...
		try {
			...
			assertThat(result).doesNotContain(testParams.newStatus).contains(testParams.oldStatus);
		} finally {
			RepositoryFileUtil.deleteFilesAndTmpDir(TMP_FILES_LOCATION);
		}
	}

	private TestParams mockForPutWithKeyAsParameterAndInvalidStatus() throws IOException {
		...
		final var newStatus = "ARE_YOU_KIDDING_ME";
